...
namespace JobLibrary.TimingTask
{
    ...
    public class StatisticsHalfYearTiming : IJob
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            try
            {
                var data = DbContext.StatisticsHalfYears.OrderBy(a => a.Id).ToList();
                ...
                {
                    item.HalfYearNumber = 0;
                    DbContext.SaveChanges();
                }
                DateTime date = DateTime.Now.AddMonths(-6).Date;
                ...
                foreach (var lotteryitem in lotterylist)
                {
                    for (int i = 1; i <= 10; i++)
                    {
                        ...
                        foreach (var item in numberlist)
                        {
                            ...
                            if (contain < 0)
                            {
                                prize.HalfYearNumber++;
                                if (prize.HalfYearNumber >= 5)
                                {
                                    StatisticsHalfYear statis = DbContext.StatisticsHalfYears.Where(s => s.Frequency == prize.HalfYearNumber && s.RankingId == i && s.NGId == item.Id).FirstOrDefault<StatisticsHalfYear>();
                                    if (statis == null)
                                    {
                                        StatisticsHalfYear statisnew = new StatisticsHalfYear();
                                        statisnew.RankingId = i;
                                        statisnew.Frequency = prize.HalfYearNumber;
                                        statisnew.Number = 1;
