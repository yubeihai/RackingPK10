...
namespace JobLibrary.TimingTask
{
    ...
    public class StatisticsThreeMonthTiming : IJob
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            try
            {
                var data = DbContext.StatisticsThreeMonths.OrderBy(a => a.Id).ToList();
                ...
                {
                    item.ThreeMonthNumber = 0;
                    DbContext.SaveChanges();
                }
                DateTime date = DateTime.Now.AddMonths(-3).Date;
                ...
                foreach (var lotteryitem in lotterylist)
                {
                    for (int i = 1; i <= 10; i++)
                    {
                        ...
                        foreach (var item in numberlist)
                        {
                            ...
                            if (contain < 0)
                            {
                                prize.ThreeMonthNumber++;
                                if (prize.ThreeMonthNumber >= 5)
                                {
                                    StatisticsThreeMonth statis = DbContext.StatisticsThreeMonths.Where(s => s.Frequency == prize.ThreeMonthNumber && s.RankingId == i && s.NGId == item.Id).FirstOrDefault<StatisticsThreeMonth>();
                                    if (statis == null)
                                    {
                                        StatisticsThreeMonth statisnew = new StatisticsThreeMonth();
                                        statisnew.RankingId = i;
                                        statisnew.Frequency = prize.ThreeMonthNumber;
                                        statisnew.Number = 1;
